...
create table if not exists public.projects (
  ...
  constraint projects_case_study_publication_matches_approval check (
    case_study_published = (case_study_approved_at is not null)
  ),
  created_at timestamptz not null default now()
);
...
create table if not exists public.contact_messages (
  ...
  locale text check (locale is null or locale in ('fr', 'en', 'de')),
  budget text check (budget is null or budget in ('<2k', '2k-5k', '5k-10k', '10k+')),
  timeline text check (timeline is null or timeline in ('urgent', '1mois', '2-3mois', 'flexible')),
  notification_status text check (notification_status is null or notification_status in ('pending', 'sent', 'failed', 'uncertain')),
  notification_provider_id text,
  notification_error_code text check (notification_error_code is null or notification_error_code in ('not_configured', 'invalid_recipient', 'provider_rejected', 'network_error', 'timeout_ambiguous', 'unknown')),
...
create or replace function public.save_project_with_publication_audit_transition(
  p_organization_id uuid,
  p_project_id bigint,
  p_actor_user_id uuid,
  p_actor_role text,
