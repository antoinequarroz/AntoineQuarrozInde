export default defineEventHandler(async (event) => {
  ...
  if (body.kind === 'project') {
    ...
    const { data, error } = await supabase.from('projects').update({ workflow_status: status, starts_at: body.startsAt || null, target_at: body.targetAt || null }).eq('organization_id', org.id).eq('id', projectId).select('*').single()
