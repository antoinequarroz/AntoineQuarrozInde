...
async function query(apiKey: string, projectId: string, name: string, hogql: string) {
...
export default defineEventHandler(async (event) => {
  ...
  const [comparison, sources, content] = await Promise.all([
    query(apiKey, projectId, 'hermes_marketing_weekly', `
    ...
    query(apiKey, projectId, 'hermes_marketing_content', `
  ...
  const recommendations = current.visitors < 20
    ? ['Conserver la collecte : moins de 20 visiteurs sur sept jours ne permettent pas encore de comparer les pages ou les canaux.']
    : [
        current.contentVisitors / current.visitors < 0.3 ? 'Renforcer les liens de l’accueil vers les articles et projets.' : 'Conserver les accès actuels vers les contenus.',
        current.contentVisitors >= 10 && current.contactIntents === 0 ? 'Tester un appel à l’action plus concret sur le contenu le plus lu.' : 'Conserver les appels à l’action pendant une nouvelle semaine.',
      ]
  return {
