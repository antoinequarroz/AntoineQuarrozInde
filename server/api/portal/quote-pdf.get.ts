...
export default defineEventHandler(async (event) => {
  ...
  const { data: quote } = await supabase.from('quotes').select('*').eq('organization_id', org.id).eq('client_id', client.id).eq('id', id).neq('status', 'draft').maybeSingle()
  ...
  const [{ data: organization }, { data: items }] = await Promise.all([
    ...
    supabase.from('quote_items').select('*').eq('organization_id', org.id).eq('quote_id', id).order('position'),
  ])
  const { pdf, engine } = await buildBillingDocument({ kind: 'quote', document: quote, organization: organization || org, client, items: items || [] })
