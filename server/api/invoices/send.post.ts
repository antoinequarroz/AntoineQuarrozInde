...
export default defineEventHandler(async (event) => {
  ...
  const { data: invoice } = await supabase.from('invoices').select('id,number,client_id,status,locked_at').eq('organization_id', org.id).eq('id', id).single()
  ...
  if (invoice.status === 'draft' || !invoice.locked_at) {
    await supabase.from('invoices').update({ status: invoice.status === 'draft' ? 'sent' : invoice.status, locked_at: invoice.locked_at || new Date().toISOString() }).eq('organization_id', org.id).eq('id', id)
  }
