...
export default defineEventHandler(async (event) => {
  ...
  await supabase.from('quote_items').delete().eq('organization_id', org.id).eq('quote_id', id)
  if (items.length) {
    const rows = items.map(item => ({ ...item, organization_id: org.id, quote_id: id }))
