...
<template>
  <div class="space-y-5">
    ...
    <div v-if="pending && !data" class="grid min-h-72 place-items-center rounded-xl border border-gray-200 bg-white dark:border-white/[0.08] dark:bg-[#111118]">
      <div class="text-center"><div class="mx-auto h-8 w-8 animate-spin rounded-full border-2 border-violet-200 border-t-violet-600" /><p class="mt-3 text-sm text-gray-500">Chargement des statistiques…</p></div>
    </div>
    ...
    <template v-if="data">
      <section class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.08] dark:bg-[#111118]" aria-labelledby="plausible-title">
        <div class="flex flex-wrap items-start justify-between gap-3 border-b border-gray-100 px-5 py-4 dark:border-white/[0.06]">
          <div><h2 id="plausible-title" class="font-display text-lg font-semibold">Audience Plausible</h2><p class="mt-1 text-xs text-gray-500">Mesure respectueuse de la vie privée, journée en cours incluse.</p></div>
        ...
        <div v-if="plausible?.totals" class="grid divide-y divide-gray-100 sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-5 dark:divide-white/[0.06]">
          <div v-for="metric in audienceMetrics" :key="metric.label" class="px-5 py-4">
        ...
        <div v-else class="m-5 rounded-lg border border-dashed border-gray-300 p-4 text-sm text-gray-500 dark:border-white/[0.12]">
      ...
      <section v-if="plausible?.totals" class="grid gap-5 xl:grid-cols-[minmax(0,1.7fr)_minmax(300px,0.8fr)]">
        <article class="min-w-0 rounded-xl border border-gray-200 bg-white p-5 dark:border-white/[0.08] dark:bg-[#111118]">
          <div class="flex flex-wrap items-start justify-between gap-3">
            <div><h2 class="font-display text-lg font-semibold">Évolution de l’audience</h2><p class="mt-1 text-xs text-gray-500">Visiteurs uniques et pages vues par jour.</p></div>
            <div class="flex items-center gap-4 text-xs text-gray-500"><span class="flex items-center gap-2"><i class="h-2.5 w-2.5 rounded-full" style="background-color:#7c3aed" />Visiteurs</span><span class="flex items-center gap-2"><i class="h-2.5 w-2.5 rounded-full" style="background-color:#22d3ee" />Pages vues</span></div>
          </div>
          <div class="mt-5 overflow-x-auto" role="img" :aria-label="`Courbe sur 30 jours : ${plausible.totals.visitors} visiteurs et ${plausible.totals.pageviews} pages vues`">
            <svg class="audience-chart min-w-[620px]" :viewBox="`0 0 ${trendChart.width} ${trendChart.height}`" aria-hidden="true">
        ...
        <article class="rounded-xl border border-gray-200 bg-white p-5 dark:border-white/[0.08] dark:bg-[#111118]">
          ...
          <p class="mt-1 text-xs text-gray-500">Répartition par source d’acquisition.</p>
          <div v-if="sourceChart.segments.length" class="mt-6 grid items-center gap-6 sm:grid-cols-[160px_1fr] xl:grid-cols-1">
            <div class="relative mx-auto h-40 w-40 rounded-full" :style="{ background: sourceChart.background }" role="img" :aria-label="`${sourceChart.total} visiteurs répartis sur ${sourceChart.segments.length} sources`">
              <div class="absolute inset-7 grid place-items-center rounded-full bg-white text-center dark:bg-[#111118]"><div><strong class="font-display text-2xl">{{ sourceChart.total }}</strong><p class="text-xs text-gray-500">visiteurs</p></div></div>
            </div>
            <div class="space-y-3">
          ...
          <p v-else class="mt-8 rounded-lg border border-dashed border-gray-200 p-5 text-center text-sm text-gray-500 dark:border-white/[0.1]">Les sources apparaîtront dès les prochaines visites.</p>
        </article>
      </section>

      <section class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.08] dark:bg-[#111118]">
        <div class="border-b border-gray-100 px-5 py-4 dark:border-white/[0.06]"><h2 class="font-display text-lg font-semibold">Actions sur le site</h2><p class="mt-1 text-xs text-gray-500">Événements internes utiles pour comprendre le parcours.</p></div>
        <div class="grid divide-y divide-gray-100 sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-6 dark:divide-white/[0.06]">
          <div v-for="metric in internalMetrics" :key="metric.label" class="px-4 py-4"><p class="text-xs text-gray-500">{{ metric.label }}</p><p class="mt-1 font-display text-xl font-semibold">{{ metric.value }}</p></div>
        </div>
      </section>
      ...
      <section class="grid gap-5 lg:grid-cols-2">
        <article class="rounded-xl border border-gray-200 bg-white p-5 dark:border-white/[0.08] dark:bg-[#111118]">
          ...
          <p class="mt-1 text-xs text-gray-500">De la découverte à la demande de contact.</p>
        ...
        <article class="rounded-xl border border-gray-200 bg-white p-5 dark:border-white/[0.08] dark:bg-[#111118]">
          ...
          <p class="mt-1 text-xs text-gray-500">Comparaison des variantes A et B du hero.</p>
          <div class="mt-6 space-y-5">
            <div v-for="variant in data.variants" :key="variant.variant">
              <div class="mb-2 flex items-end justify-between gap-3"><div><p class="text-sm font-semibold">Version {{ variant.variant }}</p><p class="text-xs text-gray-500">{{ variant.clicks }} clics sur {{ variant.views }} vues</p></div><strong class="font-display text-2xl">{{ variant.conversionRate }} %</strong></div>
          ...
          <p class="mt-6 text-xs leading-relaxed text-gray-500">Une tendance devient exploitable lorsque chaque version dispose d’un volume de visites suffisant.</p>
        </article>
      </section>
