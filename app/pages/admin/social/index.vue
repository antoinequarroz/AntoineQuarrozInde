...
async function approve() {
  ...
  try {
    ...
    closeConfirmation()
    tab.value = 'waiting'
    toast.success('Texte validé. Hermes tentera la publication à 18 h si la connexion est prête.')
  }
...
<template>
  <div class="space-y-5">
    ...
    <section v-else class="grid gap-4 xl:grid-cols-2" aria-label="Publications sociales">
      <article v-for="post in filteredPosts" :key="post.id" class="admin-card flex flex-col p-5">
        ...
        <div class="mt-auto flex flex-wrap gap-2 pt-4">
          ...
          <p v-else-if="post.status === 'approved'" class="text-xs text-gray-500">Validé · tentative de publication à 18 h si la connexion est prête.</p>
    ...
    <div v-if="confirmationPost" ref="dialogRef" class="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="social-confirm-title" tabindex="-1" @keydown="handleDialogKeydown">
      <div class="w-full max-w-lg rounded-2xl bg-white p-5 shadow-2xl dark:bg-[#15151e]"><h2 id="social-confirm-title" class="font-display text-xl font-semibold text-gray-950 dark:text-white">Valider la publication</h2><p class="mt-2 text-sm leading-6 text-gray-600 dark:text-gray-300">Après ton accord explicite, Hermes tentera de publier ce texte sur {{ platformLabel(confirmationPost.platform) }} à 18 h avec l’image de l’article. Si la connexion n’est pas prête, le texte restera en attente.</p><div class="mt-4 max-h-60 overflow-y-auto whitespace-pre-wrap rounded-xl bg-gray-50 p-4 text-sm leading-6 dark:bg-white/[0.05]">{{ confirmationPost.content }}</div><label class="mt-4 flex cursor-pointer items-start gap-3 rounded-xl border border-gray-200 p-3 dark:border-white/[0.1]"><input v-model="confirmationAccepted" type="checkbox" class="mt-1 h-4 w-4 accent-violet-600"><span class="text-sm text-gray-700 dark:text-gray-200">J’ai relu ce texte et j’autorise sa publication publique lorsque la connexion sera prête.</span></label><div class="mt-5 flex gap-3"><button data-confirm-close type="button" class="min-h-11 flex-1 rounded-lg border border-gray-200 px-4 text-sm font-semibold dark:border-white/[0.1]" @click="closeConfirmation">Annuler</button><button type="button" class="min-h-11 flex-1 rounded-lg bg-violet-600 px-4 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50" :disabled="!confirmationAccepted || busyId === confirmationPost.id" @click="approve">Valider la publication</button></div></div>
    </div>
  </div>
</template>
