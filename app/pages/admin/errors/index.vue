  ...
  <div class="space-y-5">
    <section class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-white/[0.08] dark:bg-[#111118]">
      <div class="flex flex-wrap items-start justify-between gap-4">
        ...
        <button type="button" class="min-h-11 rounded-lg bg-gray-100 px-4 text-sm font-semibold text-gray-700 hover:bg-gray-200 dark:bg-white/[0.08] dark:text-gray-100" :disabled="loading" @click="loadErrors">
        ...
        <div class="flex flex-wrap items-start justify-between gap-3">
          <div class="min-w-0">
            <div class="flex flex-wrap items-center gap-2 text-xs">
              ...
              <span class="text-gray-500">{{ error.source }}</span>
          ...
          <button type="button" class="min-h-11 rounded-lg bg-violet-600 px-3 text-xs font-semibold text-white hover:bg-violet-700" @click="resolveError(error.id)">Marquer résolue</button>
        </div>
        <details v-if="error.stack" class="mt-3">
          <summary class="cursor-pointer text-xs font-medium text-gray-600 dark:text-gray-300">Détails techniques</summary>
