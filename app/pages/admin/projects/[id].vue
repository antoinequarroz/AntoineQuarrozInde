      ...
      <section class="grid gap-5" :class="canManage ? 'xl:grid-cols-[minmax(0,1fr)_360px]' : ''">
        <div class="min-w-0 rounded-xl border border-gray-200 bg-white dark:border-white/[0.08] dark:bg-[#111118]">
          ...
          <div class="space-y-2 p-4 sm:p-5">
            ...
            <div v-if="!activeItems.length" class="py-10 text-center"><p class="text-sm font-medium text-gray-700 dark:text-gray-200">Rien ici pour le moment</p><p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ canManage ? 'Utilise le formulaire pour ajouter le premier élément.' : 'Aucun élément n’a encore été partagé dans cette section.' }}</p></div>
          </div>
        </div>
