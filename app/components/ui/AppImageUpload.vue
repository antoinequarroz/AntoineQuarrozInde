...
async function processFile(file: File) {
  ...
  const { url: publicUrl } = await $fetch<{ url: string }>('/api/admin/upload', {
    method: 'POST',
    body: { dataUrl },
...
<template>
  <div>
    <!-- Preview -->
    <div v-if="modelValue" class="relative rounded-xl overflow-hidden border border-gray-100 dark:border-white/[0.06] mb-2">
      <img :src="modelValue" class="w-full h-40 object-cover" alt="Preview">
      <button
        type="button"
    ...
    <div
      ...
      @drop.prevent="onDrop"
    >
      <svg class="w-8 h-8 mx-auto mb-2 text-gray-300 dark:text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
        <path stroke-linecap="round" stroke-linejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
      </svg>
      <p class="text-xs text-gray-400">Cliquer ou glisser une image</p>
      <p class="text-[11px] text-gray-300 dark:text-gray-600 mt-0.5">JPG, PNG, WebP — redimensionné auto</p>
    </div>
