...
describe('AQ-SEO-004 localized public pages', () => {
  ...
  it('derives homepage metadata and navigation from the active locale', async () => {
    ...
    expect(page).toContain('<SectionsPortfolioSection />')
    expect(page).toContain('projectsStore.ensureLoaded()')
    ...
    expect(header).toContain("{ key: 'portfolio', href: `${localePath('/')}#portfolio` }")
  ...
  it('uses native, crawlable language links and approved localized microcopy', async () => {
    ...
    expect(about).toContain('sm:grid-cols-[max-content_minmax(0,1fr)]')
    expect(about).toContain('min-w-0 break-words')
    expect(about).not.toContain('sm:grid-cols-[88px_1fr]')
