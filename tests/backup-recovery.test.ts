...
describe('verified backup recovery', () => {
  ...
  it('replays the backup into an unlinked disposable database without copying credentials', async () => {
    const fullDrill = await readFile(join(projectDir, 'scripts/ops/full-recovery-drill.sh'), 'utf8')
    expect(fullDrill).toContain('unset SUPABASE_ACCESS_TOKEN')
    expect(fullDrill).toContain('20260701000000_initial_schema.sql')
    expect(fullDrill).toContain('db reset --local --no-seed')
    expect(fullDrill).toContain('disable trigger user')
    expect(fullDrill).toContain('enable trigger user')
    expect(fullDrill).toContain('jsonb_populate_recordset')
    expect(fullDrill).toContain('Recovered row count mismatch')
    expect(fullDrill).toContain('db lint --local')
    expect(fullDrill).not.toContain('encrypted_password')
    expect(fullDrill).not.toMatch(/db (push|reset) --linked/)

    const remoteFullDrill = await readFile(join(projectDir, 'scripts/ops/full-recovery-supabase-copy-drill.sh'), 'utf8')
    expect(remoteFullDrill).toContain('/storage/v1/object/list/backups')
    expect(remoteFullDrill).toContain('full-recovery-drill.sh')
    expect(remoteFullDrill).not.toMatch(/\/rest\/v1\/.+-(X|--request)\s+(POST|PUT|PATCH|DELETE)/)
    expect(remoteFullDrill).not.toContain('x-upsert')
  })
})
